import { buttons, figures } from './content.js'
...
function makeCircle(ctx, { x1, x2, y1, y2, figure }) {
  ...
  ctx.ellipse(x1 + ((x2 - x1) / 2), y1 + ((y2 - y1) / 2), radX, radY, 0, 0, getRadians(360))
...
export function initCanvas() {
  ...
  function toBase() {
    opt.counter = 0
    opt.figure = null
    document.getElementById('figure').value = 'base'
  }
  ...
  canvas.addEventListener("click", function (e) {
    ...
    if (opt.figure) {
      ...
      if (opt.counter == 2 && opt.figure.indexOf('triangle') < 0) {
        if (opt.figure == 'line') { makeLine(ctx, opt) }
        if (opt.figure == 'square' || opt.figure == "squareOutline") { makeSquare(ctx, opt) }
        if (opt.figure == 'circle' || opt.figure == "circleOutline") { makeCircle(ctx, opt) }
        toBase()
      }
